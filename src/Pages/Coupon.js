...
const filteredCoupons = coupons.filter((coupon) => {
  ...
  const isDepartment = coupon.coupon_type === "department" && coupon.department_id;
  ...
  const matchesSearch = coupon.coupon_code.toLowerCase().includes(searchQuery.toLowerCase()) ||
    (isCourse && courses.some(course => course.id === coupon.course_id && course.subject_name.toLowerCase().includes(searchQuery.toLowerCase()))) ||
    (isDepartment && departments.some(dept => dept.id === coupon.department_id && dept.title.toLowerCase().includes(searchQuery.toLowerCase())));
...
const dataToDisplay = filteredCoupons;
  ...
  const getAdditionalInfo = (coupon) => {
    if (coupon.coupon_type === 'department') {
      const department = departments.find(dept => dept.id === coupon.department_id);
              ...
              <Table striped hover>
                <thead>
                  <tr className="table_head_cardprice">
                    ...
                    <th className="desc_table_cardprice">النوع</th> {/* Display coupon_type */}
                
                    <th className="desc_table_cardprice">المعلومات</th> {/* Additional info based on type */}
                    <th className="desc_table_cardprice">الاستخدام</th> {/* Display used */}
                    <th className="desc_table_cardprice">تاريخ انتهاء الكوبون</th> {/* Display expiration_date */}
                ...
                <tbody>
                  {dataToDisplay.map((couponData) => (
                    <tr key={couponData.id}>
                      ...
                      <td>{couponData.coupon_type === 'course' ? 'مادة' : 'قسم'}</td>
                     
                      <td>{getAdditionalInfo(couponData)}</td> {/* Display department or course info */}
                     
                      <td>{couponData.used ? "مستخدم" : "غير مستخدم"}</td> {/* Display used status */}
                      <td>{new Date(couponData.expiration_date).toLocaleDateString('ar-EG')}</td> {/* Format expiration_date */}
                      <td>
                        <i
